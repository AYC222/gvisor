...
func TestNICStats(t *testing.T) {
	...
	for _, nic := range nics {
		ep := channel.New(10, defaultMTU, "")
		if err := s.CreateNIC(nic.id, ep); err != nil {
			t.Fatal("CreateNIC failed: ", err)
		}
		if err := s.AddAddress(nic.id, fakeNetNumber, nic.addr); err != nil {
			t.Fatal("AddAddress failed:", err)
		}

		{
			...
			s.SetRouteTable([]tcpip.Route{{Destination: subnet, Gateway: "\x00", NIC: nic.id}})
		}

		nicStats := s.NICInfo()[nic.id].Stats
	...
	if got, want := s.Stats().NIC.Rx.Packets.Value(), uint64(rxPacketsTotal); got != want {
		t.Errorf("got s.Stats().NIC.Rx.Packets.Value() = %d, want = %d", got, want)
	}
	if got, want := s.Stats().NIC.Rx.Bytes.Value(), uint64(rxBytesTotal); got != want {
		t.Errorf("got s.Stats().Rx.Bytes.Value() = %d, want = %d", got, want)
	}
	if got, want := s.Stats().NIC.Tx.Packets.Value(), uint64(txPacketsTotal); got != want {
		t.Errorf("got Tx.Packets.Value() = %d, ep.Drain() = %d", got, want)
	}
	if got, want := s.Stats().NIC.Tx.Bytes.Value(), uint64(txBytesTotal); got != want {
		t.Errorf("got Tx.Bytes.Value() = %d, want = %d", got, want)
	}
}
