			...
			t.Run(name, func(t *testing.T) {
				for _, test := range subTests {
					t.Run(test.name, func(t *testing.T) {
						...
						// RouterOnlyPacketsDroppedByHost count should initially be 0.
						if got := routerOnly.Value(); got != 0 {
							t.Errorf("got RouterOnlyPacketsDroppedByHost = %d, want = 0", got)
						}
						...
						if test.valid && !isRouter && typ.routerOnly {
							// RouterOnlyPacketsDroppedByHost count should have increased.
							want = 1
						}
						if got := routerOnly.Value(); got != want {
							t.Errorf("got RouterOnlyPacketsDroppedByHost = %d, want = %d", got, want)
						}
					})
				}
			})
