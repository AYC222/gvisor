...
import (
	...
	net_testutil "gvisor.dev/gvisor/pkg/tcpip/network/internal/testutil"
...
func TestWriteStats(t *testing.T) {
	...
	for _, writer := range writers {
		t.Run(writer.name, func(t *testing.T) {
			for _, test := range tests {
				t.Run(test.name, func(t *testing.T) {
					ep := net_testutil.NewMockLinkEndpoint(header.IPv6MinimumMTU, &tcpip.ErrInvalidEndpointState{}, test.allowPackets)
					rt := buildRoute(t, ep)
...
func TestFragmentationWritePacket(t *testing.T) {
	...
	for _, ft := range fragmentationTests {
		t.Run(ft.description, func(t *testing.T) {
			pkt := net_testutil.MakeRandPkt(ft.transHdrLen, extraHeaderReserve+header.IPv6MinimumSize, []int{ft.payloadSize}, header.IPv6ProtocolNumber)
			source := pkt.Clone()
			ep := net_testutil.NewMockLinkEndpoint(ft.mtu, nil, math.MaxInt32)
			r := buildRoute(t, ep)
...
func TestFragmentationWritePackets(t *testing.T) {
	...
	tinyPacket := net_testutil.MakeRandPkt(header.TCPMinimumSize, extraHeaderReserve+header.IPv6MinimumSize, []int{1}, header.IPv6ProtocolNumber)
...
func TestFragmentationErrors(t *testing.T) {
	...
	for _, ft := range tests {
		t.Run(ft.description, func(t *testing.T) {
			pkt := net_testutil.MakeRandPkt(ft.transHdrLen, extraHeaderReserve+header.IPv6MinimumSize, []int{ft.payloadSize}, header.IPv6ProtocolNumber)
			ep := net_testutil.NewMockLinkEndpoint(ft.mtu, ft.mockError, ft.allowPackets)
			r := buildRoute(t, ep)
			err := r.WritePacket(stack.NetworkHeaderParams{
				Protocol: tcp.ProtocolNumber,
				TTL:      ttl,
